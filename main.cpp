...
void conversions()
{
	...
	Vector<char, true> vectorCharTrue(vectorInt);
	assert(vectorCharTrue[0] == 'Z');
	assert(vectorCharTrue[5] == 'A');
	Vector<char*, false> vectorCharStar(vectorInt);
	assert(*(vectorCharStar[0]) == 'Z');
	assert(*(vectorCharStar[5]) == 'A');

	Vector<int*, false> vectorIntStar(10);
	*(vectorIntStar[0]) = 90;
	*(vectorIntStar[5]) = 65;
	Vector<char*, false> vectorCharStar2(vectorIntStar);
	assert(*(vectorCharStar2[0]) == 'Z');
	assert(*(vectorCharStar2[5]) == 'A');
	Vector<char, true> vectorCharTrue2(vectorIntStar);
	assert(vectorCharTrue[0] == 'Z');
	assert(vectorCharTrue[5] == 'A');
	Vector<char, false> vectorChar2(vectorIntStar);
	assert(vectorChar2[0] == 'Z');
	assert(vectorChar2[5] == 'A');

	Vector<int, true> vectorIntTrue(10);
	vectorIntTrue[0] = 90;
	vectorIntTrue[5] = 65;
	Vector<char*, false> vectorCharStar3(vectorIntTrue);
	assert(*(vectorCharStar2[0]) == 'Z');
	assert(*(vectorCharStar2[5]) == 'A');
	Vector<char, true> vectorCharTrue3(vectorIntTrue);
	assert(vectorCharTrue[0] == 'Z');
	assert(vectorCharTrue[5] == 'A');
	Vector<char, false> vectorChar3(vectorIntTrue);
	assert(vectorChar2[0] == 'Z');
	assert(vectorChar2[5] == 'A');
}
